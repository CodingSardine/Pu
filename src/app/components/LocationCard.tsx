...
import { X, MapPin, Clock, UtensilsCrossed, DollarSign, Wind, Armchair, ExternalLink, Star } from 'lucide-react';
...
function EatIconBadge({ className = 'w-11 h-11' }: { className?: string }) {
  return (
    <svg className={`block ${className}`} fill="none" preserveAspectRatio="none" viewBox="0 0 110 110">
      <g>
...
function FocusIconBadge({ className = 'w-11 h-11' }: { className?: string }) {
  return (
    <svg className={`block ${className}`} fill="none" preserveAspectRatio="none" viewBox="0 0 110 110">
      <g>
...
function ChillIconBadge({ className = 'w-11 h-11' }: { className?: string }) {
  return (
    <svg className={`block ${className}`} fill="none" preserveAspectRatio="none" viewBox="0 0 110 110">
      <g>
  ...
  return (
    ...
    >
      ...
      {images.length > 1 && (
        <>
          <div
            className={`absolute bottom-3 left-1/2 -translate-x-1/2 z-20 flex items-center gap-1.5 rounded-full px-2.5 py-1.5 ${
              theme === 'dark' ? 'bg-black/35' : 'bg-white/45'
            } backdrop-blur-md border ${theme === 'dark' ? 'border-white/10' : 'border-black/10'}`}
          >
            {images.map((_, index) => (
              <button
                key={index}
                ...
                className={`h-2.5 rounded-full transition-all duration-300 ${
                  index === currentIndex ? 'w-7' : 'w-2.5 opacity-70'
                }`}
                style={{
                  backgroundColor:
                    index === currentIndex
...
function GenericLocationCard({
  ...
  const iconMutedClass = theme === 'dark' ? 'text-slate-300' : 'text-slate-600';
  const dividerClass = theme === 'dark' ? 'border-white/10' : 'border-black/10';
  ...
  return (
    <div className="relative w-full" style={{ animation: 'fadeIn 0.2s ease-out' }}>
      ...
      <div
        className={`${surfaceClass} content-stretch flex flex-col items-start overflow-clip relative rounded-2xl sm:rounded-[10px] shadow-[0px_16px_40px_-18px_rgba(0,0,0,0.55),0px_10px_18px_-10px_rgba(0,0,0,0.35)] w-full backdrop-blur-xl`}
      >
        {/* Image area — responsive height */}
        {images.length > 0 && (
          <div
            className={`relative shrink-0 w-full h-24 sm:h-32 lg:h-36 ${
        ...
        {/* Close button */}
        <button
          className={`absolute cursor-pointer flex items-center justify-center rounded-full size-[34px] sm:size-[32px] z-20 right-[12px] top-[10px] sm:top-[12px] shadow-lg transition-colors ${
            theme === 'dark'
              ? 'bg-slate-900/70 hover:bg-slate-900/90 border border-white/10'
              : 'bg-white/85 hover:bg-white border border-black/10'
          }`}
          onClick={onClose}
          aria-label="Close"
        >
          <X size={16} className={theme === 'dark' ? 'text-slate-100' : 'text-slate-700'} />
        </button>
        ...
        <div className="relative shrink-0 w-full overflow-y-visible sm:overflow-y-auto max-h-none sm:max-h-[calc(100vh-14rem)]">
          <div className="content-stretch flex flex-col gap-[8px] items-start pt-[16px] sm:pt-[20px] px-[16px] sm:px-[20px] relative w-full">
            ...
            <div className="flex items-center gap-3 w-full pr-10">
              <IconBadge className="w-11 h-11 shrink-0" />

              <div className="min-w-0">
                <p className="font-['Arimo:Bold',sans-serif] font-bold leading-[22px] sm:leading-[26px] text-[18px] sm:text-[20px] tracking-[-0.01em] break-words">
                  {location.name}
                </p>
              </div>
            </div>
            ...
            {displayDescription && (
              <p className={`font-normal leading-[20px] text-[14px] ${mutedTextClass}`}>
                {displayDescription}
              </p>
            )}
            ...
            {placeData?.rating != null && (
              <div className="flex items-center gap-[6px]">
                ...
                {placeData.userRatingCount != null && (
                  <span className={`font-normal text-[12px] leading-[20px] ${subtleTextClass}`}>
            ...
            {displayAddress && (
              <div className="flex items-center gap-[8px] relative shrink-0 w-full">
                <MapPin size={16} className={`${iconMutedClass} shrink-0`} strokeWidth={1.33} />
                <p className={`font-normal leading-[20px] text-[14px] ${mutedTextClass}`}>{displayAddress}</p>
              </div>
            )}

            {displayHours && (
              <div className="content-stretch flex gap-[8px] items-center relative shrink-0 w-full">
                <Clock size={16} className={`${iconMutedClass} shrink-0`} strokeWidth={1.33} />
                <p className={`font-normal leading-[20px] text-[14px] ${mutedTextClass}`}>{displayHours}</p>
              </div>
            )}

            <div className="content-stretch flex flex-col items-start pt-[16.8px] relative shrink-0 w-full">
              <div aria-hidden="true" className={`absolute border-solid border-t inset-x-0 top-0 pointer-events-none ${dividerClass}`} />
              <div className="grid grid-cols-2 gap-y-[12px] w-full">
                {location.cuisine && (
                  <div className="flex gap-[8px] items-center">
                    <UtensilsCrossed size={20} className={`${subtleTextClass} shrink-0`} strokeWidth={1.67} />
                    <div>
                      <p className={`font-normal leading-[16px] text-[12px] ${subtleTextClass}`}>Cuisine</p>
                      <p className="font-normal leading-[20px] text-[14px]">{location.cuisine}</p>
                    </div>
                  </div>
                )}
                {location.price && (
                  <div className="flex gap-[8px] items-center">
                    <DollarSign size={20} className={`${subtleTextClass} shrink-0`} strokeWidth={1.67} />
                    <div>
                      <p className={`font-normal leading-[16px] text-[12px] ${subtleTextClass}`}>Price</p>
                      <p className="font-normal leading-[20px] text-[14px]">{location.price}</p>
                    </div>
                  </div>
                )}
                {location.atmosphere && (
                  <div className="flex gap-[8px] items-center">
                    <Wind size={20} className={`${subtleTextClass} shrink-0`} strokeWidth={1.67} />
                    <div>
                      <p className={`font-normal leading-[16px] text-[12px] ${subtleTextClass}`}>Atmosphere</p>
                      <p className="font-normal leading-[20px] text-[14px]">{location.atmosphere}</p>
                    </div>
                  </div>
                )}
                {location.seating && (
                  <div className="flex gap-[8px] items-center">
                    <Armchair size={20} className={`${subtleTextClass} shrink-0`} strokeWidth={1.67} />
                    <div>
                      <p className={`font-normal leading-[16px] text-[12px] ${subtleTextClass}`}>Seating</p>
                      <p className="font-normal leading-[20px] text-[14px]">{location.seating}</p>
                    </div>
                  </div>
                )}
              </div>
            ...
            {(placeData?.googleMapsUri || placeData?.placeId || location.gmaps) && (
              <div className="pb-[20px] w-full">
                <a
                  ...
                  className={`flex items-center justify-center gap-2 w-full py-2.5 rounded-xl text-[13px] font-semibold transition-all active:scale-[0.99] ${
                    theme === 'dark' ? 'text-slate-950' : 'text-white'
                  }`}
                  style={{
                    backgroundColor: color,
                    boxShadow: `0 10px 24px ${color}25, 0 6px 12px ${color}20`,
                  }}
                >
